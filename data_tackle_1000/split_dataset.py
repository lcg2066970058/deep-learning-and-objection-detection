...
print("识别到%d个类别：%s" % (len(class_names), class_names))
print("类别ID映射：%s" % class_to_id)
...
for class_name in class_names:
    ...
    print("类别%s：共%d张图片" % (class_name, total_num))
...
# 运行完成提示
print("="*60)
print("数据集拆分完成！生成路径：%s" % os.path.abspath(TARGET_DATA_DIR))
print("最终生成的目录结构：")
print("%s/" % TARGET_DATA_DIR)
print("  images/  (train/val/test 拆分后的图片)")
print("  labels/  (后续标注文件存放位置)")
print("  classes.txt  (类别列表)")
print("  dataset.yaml (YOLO训练专用配置文件)")